...
Joomla.overrider.searchStrings = function(more) {
	...
	if (!more) {
		...
		this.states.searchtype = document.id('jform_searchtype') !== null ? document.id('jform_searchtype').value : 'value';
	}
	...
	var req = new Request.JSON({
		...
		onSuccess: function(r) {
			...
			if (r.data) {
				...
				if (r.data.more) {
					// If there are more results than the sent ones, display the more link
